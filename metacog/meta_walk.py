...
def meta_thought(
    ...
    *,
    ...
    prompt = (
        ...
        "If the fact carries IRONY / sarcasm or exaggerated SUPERLATIVES / "
        "propaganda register echoed by a non-official voice (e.g. 'supreme "
        "leader', 'obliterated', 'decimated') note the author's INTENDED (not "
        "literal) meaning ; if there is no such signal, reflect plainly as "
        "usual — do not force irony. Plain sentence, no headers, no markdown.\n\n"
        f"{query_block}"
        f"{chain_block}"
        f"{accum_block}"
        f"{meta_line}\n"
