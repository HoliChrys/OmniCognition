...
def nearest_facts_with_fallback(
    ...
    *,
    ...
    entity_anchors: Optional[List[str]] = None,
    ...
    entity_anchors — list of entity name strings (e.g. ["Jon", "Gina"]).
    For multi-hop comparison questions ("what do X and Y have in common?")
    the full question's BM25 signal is weak because it asks about the
    INTERSECTION, not about X or Y individually. A per-entity BM25 pass
    ensures every turn that mentions X (even as speaker prefix "Jon: …")
    and every turn that mentions Y rank highly. RRF-merged at every stage.
    ...
    if encoder is not None and extractor is not None:
        ...
        if has_anchor or has_hyde:
            ...
            fact_pts_bm25 = [p for p in search_points if p.kind == PointKind.FACT]
            if has_anchor:
                for rank, (_, p) in enumerate(
                    bm25_score(anchor_query, fact_pts_bm25, k_pool=overfetch)
                ):
                    rrf_scores[p.id] = rrf_scores.get(p.id, 0.0) + 1.0 / (rrf_k + rank)
                    pid_map.setdefault(p.id, p)
            if has_hyde:
...
class MetaWalker:
    ...
    def __init__(
        ...
        *,
        ...
        # Entity anchors — proper nouns / names extracted from the query.
        # Each gets its own BM25 pass at every walk stage so multi-hop
        # comparison questions ("what do X and Y have in common?") surface
        # turns that mention X and turns that mention Y independently —
        # the combined-query BM25 is weak for the intersection question.
        # Fast regex, no LLM, common question-word exclusion list.
        _Q_STOP = {
            "What", "When", "Where", "Who", "Which", "How", "Why",
            "Would", "Could", "Did", "Does", "Has", "Have", "Are",
            "Were", "Will", "Can", "The", "And", "But", "For", "With",
            "From", "That", "This", "They", "Both", "Also", "Their",
            "About", "After", "Before", "During", "While", "Since",
        }
        import re as _re
        self._entity_anchors: List[str] = [
            w for w in _re.findall(r'\b[A-Z][a-zA-Z]{2,}\b', query)
            if w not in _Q_STOP
        ]
    ...
    def step(self) -> StageOutput:
        ...
        facts = nearest_facts_with_fallback(
            ...
            entity_anchors=self._entity_anchors or None,
